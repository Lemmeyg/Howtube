-- First, check current admin status
SELECT id, email, is_admin, subscription_tier
FROM public.profiles
WHERE id = 'f02f1b90-0c87-42f0-be23-36dab481e0eb';

-- Update admin status and subscription tier if needed
UPDATE public.profiles
SET 
    is_admin = true,
    subscription_tier = 'enterprise'::subscription_tier,
    updated_at = NOW()
WHERE id = 'f02f1b90-0c87-42f0-be23-36dab481e0eb'
AND (is_admin = false OR subscription_tier != 'enterprise'::subscription_tier)
RETURNING id, email, is_admin, subscription_tier; 